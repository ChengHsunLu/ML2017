...
modelfile = './model_selftraining_' + str(self_iter) + '_v8.h5'
...
if __name__ == '__main__':
	...
	## training
	for n in range(self_iter):
		if (n==0):
			print("=================================")
			print("===== my model generating =======")
			print("=================================")
			model = gen_model()
			model.summary()
			...
			model.fit(xtrain, ytrain, batch_size=batch, epochs=epoch, shuffle=True)
			model.save(modelfile)
			xtest_new = xtest
			xtrain_new = xtrain
			ytrain_new = ytrain
		else:
			if (xtest_new.shape[0] != 0):
				xtest_new, xtrain_new, ytrain_new = label_test(xtest_new, xtrain_new, ytrain_new, model, threshold=0.98)
				print('self_training @ iteration = ', str(n))
				print('test_size = ', xtest_new.shape)
				print('train_size = ', xtrain_new.shape)
				model = gen_model()
				model.fit(xtrain_new, ytrain_new, batch_size=batch, epochs=epoch, shuffle=True)
				model.save(modelfile)
			else:
				break
			
	print("=================================")
	print("=== the final train/test size ===")
	print("=================================")
	print('test_size = ', xtest_new.shape)
	print('train_size = ', xtrain_new.shape)
	## predicting
